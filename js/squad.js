...
//   ・階を移っても分隊はそのまま（新しい階でも独立行動を続ける。呼び戻すまで続く）
//   ・分隊の子は「主人公についてくる仲間」の枠（maxAllies）に数えない。そのぶん新しい仲間を連れて歩ける
...
Game.squad = {
  ...
  // 階を移る時：分隊はそのまま。行き先だけ新しい階で決め直す（新しい階でも独立行動を続ける）
  onNewFloor: function () {
    var sqs = this.list();
    ...
    if (sqs.length > 0) Game.log.add("分隊も一緒に階を移り、この階でも独立行動を続ける。", "ally");
  },
