...
//   # = 壁   . = 床   > = 下り階段   O = 脱出口
//   （拠点用） , = 牧場の草地   C = 収納箱 / H = 交配小屋 / K = 掲示板 / Z = 図鑑 / S = 記録の石碑（入れない。触れると開く）   G = ダンジョンへの門
Game.map = {
  tiles: [], // tiles[y][x]
  width: 0,
  height: 0,
  ...
  // (x, y) のマスに入れるか？ マップ外・壁・拠点の設備（収納箱・交配小屋・掲示板・石碑・図鑑・案内板）は入れない
  isWalkable: function (x, y) {
    var t = this.tileAt(x, y);
    return t !== "#" && t !== "C" && t !== "H" && t !== "K" && t !== "S" && t !== "Z" && t !== "T";
  },
